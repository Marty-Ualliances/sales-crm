...
export default function EmailOutreachPage() {
    ...
    const contactedLeads = useMemo(() => leadsWithEmail.filter((l: any) => l.status !== 'New Lead'), [leadsWithEmail]);
                            ...
                            {isLoading ? (
                                ...
                                leadsWithEmail.map((lead: any) => (
                                    <tr key={lead.id} className="border-b border-border/50 hover:bg-muted/30">
                                        ...
                                        <td className="px-4 py-3">
                                            <Badge variant={lead.status === 'Closed Won' ? 'default' : lead.status === 'New Lead' ? 'secondary' : 'outline'}>
                                                {lead.status}
                                            </Badge>
                                        </td>
